...
STUDENT_TEST("A-Sequence of positive order should start with 'A'"){
    for(int i = 0 ; i < 10 ; i++){
        string as = aSequenceOfOrder(i);
        string bs = bSequenceOfOrder(i);
        EXPECT(as[0]=='A' && bs[0] == 'B');
    }
}

STUDENT_TEST("A-sequence of positive order should have 2^(n-1) As") {
    for (int i = 1; i < 10; i++) {
        int as = 0;
        for (char ch: aSequenceOfOrder(i)) {
            if (ch == 'A') as++;
